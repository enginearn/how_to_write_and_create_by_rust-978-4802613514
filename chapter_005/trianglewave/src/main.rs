...
fn main() {
    ...
    let mut writer = hound::WavWriter::create("trianglewave.wav", spec).unwrap();
...
fn calc_len(bpm: usize, len: usize) -> usize {
    ((60.0 / bpm as f32) * (4.0 / len as f32) * SAMPLE_RATE) as usize
}

fn tri_wave(noteno: i32, len: usize, gain: f32) -> Vec<f32> {
    let tone = noteno_to_hz(noteno); // tone
    let form_samples = SAMPLE_RATE / tone; // 1 wave length
    ...
    wav.iter().map(|v| (v * gain) as f32).collect()
}
